...
app.get("/success", async (req, res) => {
  if (!req.query.code) return res.send("Мда");
    try {
      const r = await fetch("https://oauth.di-api.net.ru/token", {
        method: "POST",
        body: JSON.stringify({
          client_id: "5fd13b3c45a6ee51f1a6f4d3",
          client_secret: "2967104",
          redirect_uri: "https://hh.di-api.net.ru/success",
      ...
      const r2 = await fetch("https://oauth.di-api.net.ru/api/user", {
      ...
      if (allScopes.includes("balance"))
        text.push(`Твой баланс: <code>${user.balance} АР</code>`);
...
app.get('/buy', (req, res) => {
  res.send(`
    <body style='height:100vh; width:100%; display:flex; flex-direction:column; align-items:center; justify-content:center;'>
      <h1>PLEASE</h1>
      <p>BUY THIS SHIT!!! <a href='https://main.di-api.net.ru/app/suheugene/8?text=Шалкер&uid=123'>CLICK</a></p>
    </body>
  `);
});
